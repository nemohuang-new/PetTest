...
export const ACTION_TYPES = {
  ...
  PARTIAL_UPDATE_PET: 'pet/PARTIAL_UPDATE_PET',
...
export default (state: PetState = initialState, action): PetState => {
  switch (action.type) {
    ...
    case REQUEST(ACTION_TYPES.PARTIAL_UPDATE_PET):
      return {
        ...state,
        errorMessage: null,
        updateSuccess: false,
    ...
    case FAILURE(ACTION_TYPES.PARTIAL_UPDATE_PET):
    ...
    case SUCCESS(ACTION_TYPES.PARTIAL_UPDATE_PET):
      return {
        ...state,
        updating: false,
        updateSuccess: true,
...
export const getEntities: ICrudGetAllAction<IPet> = (page, size, sort) => {
  ...
  return {
    ...
    payload: axios.get<IPet>(`${requestUrl}${sort ? '&' : '?'}cacheBuster=${new Date().getTime()}`),
  };
};
...
export const partialUpdate: ICrudPutAction<IPet> = entity => async dispatch => {
  const result = await dispatch({
    type: ACTION_TYPES.PARTIAL_UPDATE_PET,
