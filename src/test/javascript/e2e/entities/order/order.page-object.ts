...
export class OrderDeleteDialog {
  ...
  private dialogTitle: ElementFinder = element(by.id('nemo1App.order.delete.question'));
...
export default class OrderComponentsPage {
  ...
  async deleteOrder() {
    ...
    const orderDeleteDialog = new OrderDeleteDialog();
    await waitUntilDisplayed(orderDeleteDialog.deleteModal);
    expect(await orderDeleteDialog.getDialogTitle().getAttribute('id')).to.match(/nemo1App.order.delete.question/);
